...
func TestFeatureSubmissionCV(t *testing.T) {
	...
	t.Run("Must not submit the CV if it is not validated.", func(t *testing.T) {
		EventBus := event.NewEventBus()		
		CVRepository := adapters.NewInmemoryCVRepository()
		CVService := NewCVSubmissionService(CVRepository, EventBus)
	...
	t.Run("Must publish the CVSubmetido event", func(t *testing.T) {
		EventBus := event.NewEventBus()
		isPublished := false
	...
	t.Run("Must not publish the CVSubmetido event if the CV is not submited.", func(t *testing.T) {
		EventBus := event.NewEventBus()
		isPublished := false
		var eventHandler = event.HandlerFunc(func(e event.Event) {
			if e.Name() == "CVSubmetido" {
		...
		if isPublished {
			t.Errorf("I was hoping that the CVSubmetido event would not be published.")
		}
	})
}
